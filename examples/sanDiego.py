...
class prometheus_thermochemistry_kernel():
    ...
    def get_mixture_specific_heat_cp_mass(self, T, Y):
        ...
        cp += Y[7] * cp0_R[7] * self.iwts[7]
        cp += Y[8] * cp0_R[8] * self.iwts[8]
    ...
    def get_mixture_enthalpy_mass(self, T, Y):
        ...
        h += Y[7] * h0_RT[7] * self.iwts[7]
        h += Y[8] * h0_RT[8] * self.iwts[8]
    ...
    def get_mixture_internal_energy_mass(self, T, Y):
        ...
        e += Y[7] * e0_RT[7] * self.iwts[7]
        e += Y[8] * e0_RT[8] * self.iwts[8]
    ...
    def get_species_enthalpies_RT(self, T):
        ...
        h0_RT[7] = np.where( tt0 < 1.000000e+03, h_low, h_high )
        ...
        h0_RT[8] = np.where( tt0 < 1.000000e+03, h_low, h_high )
    ...
    def get_falloff_rates(self, T, C, k_fwd):
        ...
        work[0] = (1.0 - 5.000000e-01) * np.exp( - 1.000000e+30 * T) + 5.000000e-01 * np.exp( - 1.000000e-30 * T) + np.exp( - 1.000000e-17 * inv_T)
        work[1] = (1.0 - 4.300000e-01) * np.exp( - 1.000000e-30 * T) + 4.300000e-01 * np.exp( - 1.000000e+30 * T) + np.exp( - 1.000000e-17 * inv_T)
    ...
    def get_rate_coefficients(self, T, C):
        ...
        k_fwd[0] = np.exp(3.119207e+01 - 7.000000e-01 * log_T - 8.589852e+03 * inv_T)
        k_fwd[1] = np.exp(3.923952e+00 + 2.670000e+00 * log_T - 3.165568e+03 * inv_T)
        k_fwd[2] = np.exp(1.397251e+01 + 1.300000e+00 * log_T - 1.829343e+03 * inv_T)
        k_fwd[3] = np.exp(6.551080e+00 + 2.330000e+00 * log_T - 7.320978e+03 * inv_T)
        k_fwd[4] = np.exp(2.789339e+01 - 1.000000e+00 * log_T)
        k_fwd[5] = np.exp(3.822766e+01 - 2.000000e+00 * log_T)
        k_fwd[6] = np.exp(2.254296e+01 - 5.000000e-01 * log_T)
        k_fwd[7] = np.exp(2.918071e+01 - 1.000000e+00 * log_T)
        k_fwd[8] = np.exp(2.280271e+01)
        k_fwd[10] = np.exp(2.498312e+01 - 1.484161e+02 * inv_T)
        k_fwd[11] = np.exp(2.353267e+01 - 4.140977e+02 * inv_T)
        k_fwd[12] = np.exp(2.415725e+01 - 8.659610e+02 * inv_T)
        k_fwd[13] = np.exp(2.371900e+01)
        k_fwd[14] = np.exp(2.683251e+01 - 5.500055e+03 * inv_T)
        k_fwd[15] = np.exp(2.411777e+01 + 2.501665e+02 * inv_T)
        k_fwd[17] = np.exp(1.908337e+01 + 7.090055e+02 * inv_T)
        k_fwd[18] = np.exp(2.535799e+01 - 5.556583e+03 * inv_T)
        k_fwd[19] = np.exp(2.385876e+01 - 4.000619e+03 * inv_T)
        k_fwd[20] = np.exp(2.302585e+01 - 1.804085e+03 * inv_T)
        k_fwd[21] = np.exp(2.505268e+01 - 3.659888e+03 * inv_T)
        k_fwd[22] = np.exp(2.127715e+01 - 1.599622e+02 * inv_T)
        k_fwd[23] = np.exp(9.172639e+00 + 2.000000e+00 * log_T - 2.008548e+03 * inv_T)
